...
## Project: Climate × Agriculture (Farmland Transactions) — bootstrapped 2026-05-19

[LEARN:project] **This project is Stata + LaTeX, targeting JEEM.** Working title *"Climate Change and Agriculture: Evidence from Farmland Transactions"* (Ricardian / hedonic tradition). Repo was forked from `pedrohcgs/claude-code-my-workflow` v1.8.0 (originally a slides + R toolkit) and adapted on 2026-05-19. Adaptation captured in `quality_reports/decisions/2026-05-19_workflow-adaptation-for-jeem-paper.md`.

[LEARN:design] **Tables flow Stata → `Tables/tab_*.tex` → `\input{}` into `Paper/main.tex`. Figures flow Stata → `Figures/fig_*.pdf` → `\includegraphics{}` into `Paper/main.tex`.** Never hand-edit a number, table, or figure in `Paper/`. Single source of truth: a fix in `Stata/do/04_analyze.do` propagates to the paper automatically on next `00_run_all.do` + xelatex pass. Why: prevents the "I updated the analysis but forgot to update Table 2" bug — exactly what `/audit-reproducibility` is designed to catch.

[LEARN:design] **Stata figure palette matches `Preambles/header.tex` via the `grstyle` scheme set in `Stata/profile.do`.** 9 series colors hard-coded: primary-blue `#012169` first, primary-gold `#B9975B` second, then highlight-yellow, positive, negative, neutral, hi-slate, hi-green, hi-red. If a figure looks like Stata's default `s2color` scheme, `grstyle` was not loaded (likely missing `ssc install grstyle, replace`).

[LEARN:design] **`Data/raw/` is GITIGNORED. Never commit raw farmland-transaction data** (CoreLogic / ZTRAX / county assessor extracts under DUA). `Data/derived/` may be committed if (a) safe to publish, (b) small (<25MB each, <100MB total). Default `.gitignore` rule blocks `.dta`, `.csv`, `.parquet` under `Data/derived/`; whitelist specific files via `!Data/derived/<filename>` when you decide they should ship. Privacy guard.

[LEARN:design] **Stata code conventions live in `.claude/rules/stata-code-conventions.md`; reviewer agent is `stata-reviewer`; user-facing skill is `/review-stata`.** Mirrors the R triple (`r-code-conventions.md` / `r-reviewer` / `/review-r`). Cross-artifact-review.md auto-dispatches `/review-r` on `.R` files and `/review-stata` on `.do`/`.ado` files when `/review-paper` runs on a manuscript that `\input{}`s those files. Audit-reproducibility skill knows about Stata `esttab .tex`, `reghdfe .log`, `csdid .log`, and `_b[]/_se[]` `.csv` summary patterns.

[LEARN:design] **Staggered-DiD discipline is a load-bearing convention for this project.** Vanilla TWFE (`reghdfe y D, absorb(unit time)`) is **banned** for staggered treatment; `stata-reviewer` flags it as Critical. Use `csdid` (Callaway-Sant'Anna), `did_imputation` (Borusyak-Jaravel-Spiess), `eventstudyinteract` (Sun-Abraham), or `did_multiplegt` (de Chaisemartin-D'Haultfœuille). Justify the choice in a comment. Why: heterogeneous-effects bias is the methodological story of the 2020-2024 applied-micro literature; JEEM referees will catch it.

[LEARN:workflow] **First few sessions on this project: check in with the user more often.** User is new to the Claude Code workflow and asked to learn how it operates. Default behavior for the first ~3-5 sessions: surface decisions and propose options rather than commit silently to defaults. Standard "contractor mode after plan approval" still applies — just lean toward more pauses at meaningful junctures (before commits, before destructive operations, after major milestones).

[LEARN:project] **Raw data lives in a host folder named `climate on agriculture`** (exact path TBD — user to confirm). Reach into `Data/raw/` by symlink or copy from there; never commit. See `Data/README.md` "Project data folder" section. When the user gives the absolute path, update both files.
